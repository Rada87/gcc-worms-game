...
const DefaultRules: Omit<
  Required<GameRules>,
  "winWhenAllObjectsOfTypeDestroyed"
> = {
...
describe("GameState", () => {
  test("requires at least one team", () => {
    expect(
      () =>
        new GameState(
          [],
          { entitiesMoving$: of(false) } as Partial<GameWorld> as GameWorld,
          DefaultRules,
        ),
    ).toThrow();
  });
  test("should be able to get active teams", async () => {
    const gameState = new GameState(
      [RED_TEAM, { ...BLUE_TEAM, worms: [DEAD_WORM] }],
      { entitiesMoving$: of(false) } as Partial<GameWorld> as GameWorld,
      DefaultRules,
    );
  ...
  describe("Round progression", () => {
    ...
    test("should advance round to the next team", () => {
      gameState = new GameState(
        [RED_TEAM, BLUE_TEAM],
        { entitiesMoving$: entitiesMoving } as Partial<GameWorld> as GameWorld,
        DefaultRules,
      );
      const [redTeam, blueTeam] = gameState.getActiveTeams();
      gameState.begin();
      expect(completeRound(gameState)).toEqual({
        nextTeam: redTeam,
        nextWorm: redTeam.worms[0],
      });
      expect(completeRound(gameState)).toEqual({
        nextTeam: blueTeam,
        nextWorm: blueTeam.worms[0],
      });
    });
    test("should advance round to the next team, skipping over the same group", () => {
      gameState = new GameState(
        [RED_TEAM, RED_TEAM_2, BLUE_TEAM],
        { entitiesMoving$: entitiesMoving } as Partial<GameWorld> as GameWorld,
        DefaultRules,
      );
    ...
    test("should detect a win when only one group has active worms", () => {
      ...
      expect(nextState).toMatchObject({ winningTeams: [redTeam, redTeam2] });
      expect("toast" in nextState ? nextState.toast : undefined).toBeDefined();
    });
    ...
    test("should handle the first round", async () => {
      ...
      expect(await getFinalValue(gameState.remainingRoundTimeSeconds$)).toEqual(
        5,
      );
    });
    ...
    test("should handle the player moving (using playCurrentRound) during preround", async () => {
      ...
      expect(await getFinalValue(gameState.roundState$)).toEqual(
        RoundState.Playing,
      );
      expect(gameState.paused).toEqual(false);
      expect(await getFinalValue(gameState.remainingRoundTimeSeconds$)).toEqual(
        DefaultRules.roundDurationMs / 1000,
      );
    });
  ...
  describe("Toast calculation", () => {
    ...
    beforeEach(() => {
      ...
      gameState = new GameState(
        [RED_TEAM, BLUE_TEAM, YELLOW_TEAM],
        world,
        DefaultRules,
      );
      gameState.begin();
    ...
    test("should show correct toast when an enemy team is killed", () => {
      ...
      const { toast } = gameState.advanceRound();
      expect(
        FireResultKilledEnemyTeam.map((v) =>
          templateText(v, {
            WormName: redTeam.worms[0].name,
